...
class ExternKalmanFilter(object):
    ...
    def update(self, p_true, R_true, delta_t=0):
        ...
        for i in range(len(self.map_points)):
            ...
            min_z = 0.1
            max_norm = 1.
            if (point_c_true[2] > min_z and point_c_pred[2] > min_z or 1):
                p1 = np.array(
        ...
        for i in range(len(obs_pred)):
            ...
            p_wrt_Rw = hat(vector)
            # about position
            H[i * 3:i * 3 + 3, :3] = (p_wrt_tw)
            H[i * 3:i * 3 + 3, 3:6] = -R_pred * delta_t
            # obs w.r.t rotation
            H[i * 3:i * 3 + 3, 6:] = (p_wrt_Rw)

        # obs noise to each obs
        #     nx1 ny1 nx2 ny2 nx3 ny3 ...
        # nx1  1
        # ny1      1
        # ...
        R = np.eye(3 * len(obs_pred))
        R *= self.obs_noise
        ...
        # be careful for the array calculation
        self.x_k = self.x_k + (K.dot(
            (obs_true - obs_pred).reshape(-1, 1))).reshape(1, -1)[0]
    ...
    def run(self, imu_pose_data):
        ...
        while idx < len(imu_pose_data) - 1:
            ...
            # update
                ...
                print('cov and norm: ', np.diag(self.P_k), ' ',
                      np.linalg.norm(np.diag(self.P_k)))
